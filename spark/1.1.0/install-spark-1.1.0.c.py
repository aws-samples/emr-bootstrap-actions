...
spark_archive = "spark-{0}.c.tgz".format(spark_version)
...
def download_and_uncompress_files():
	...
	subprocess.check_call(["/bin/ln","-s",hadoop_apps+"/spark-" + spark_version, spark_home])
	# cleanup 
...
def prepare_classpath():
	...
	emr_fs = os.path.join(spark_classpath,"emr-fs")
	subprocess.check_call(["/bin/mkdir","-p",spark_classpath])
	subprocess.check_call(["/bin/cp","-R","/usr/share/aws/emr/emr-fs/lib/",emr_fs])
...
def config():
	...
	with open(spark_env_tmp_location,'a') as spark_env:
		...
		spark_env.write("export SPARK_CLASSPATH=\"{0}/emr/*:{1}/emr-fs/*:{2}/share/hadoop/common/lib/*:{3}:/home/hadoop/hive/conf/*\"\n".format(spark_classpath,spark_classpath,hadoop_home,lzo_jar))
